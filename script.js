...
setTimeout(() => {
    ...
    function initStars() {
        ...
        stars = [];
        // Number of stars based on screen size for realism
        const numStars = Math.floor((w * h) / 1200); 
        for(let i = 0; i < numStars; i++) {
            stars.push({
                x: Math.random() * w,
                y: Math.random() * h,
                r: Math.random() * 1.5, // Tiny realistic size
                a: Math.random(), // Alpha/Opacity
                s: Math.random() * 0.3 + 0.05 // Very slow upward drift
            });
        }
    }
    ...
    function drawStars() {
        ...
        stars.forEach(star => {
            ...
            ctx.fillStyle = (isLightMode ? 'rgba(0,0,0,' : 'rgba(255,255,255,') + star.a + ')';
            
            // Add subtle glow to bigger stars
            if (star.r > 1) {
                ctx.shadowBlur = 5;
                ctx.shadowColor = isLightMode ? 'rgba(0,0,0,0.8)' : 'rgba(255,255,255,0.8)';
            } else {
                ctx.shadowBlur = 0;
            }
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        stars.forEach(star => {
            // Bigger stars move slightly faster (depth effect)
            star.x += (mouseX - w/2) * 0.00003 * star.r;
            star.y += (mouseY - h/2) * 0.00003 * star.r;
        });
    });
}, 500);
